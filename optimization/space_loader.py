...
class SearchSpaceLoader:
    def __init__(self, components_config: Dict[str, Any]):
        ...
        opt_logger.info("Loading and parsing reaction components from config...")
        ...
        for name, details in self.config.items():
            ...
            if mode == 'search':
                ...
                if df is not None:
                    ...
                    opt_logger.info(f"  - Loaded SEARCH component '{name}' with {len(df)} options.")
                else:
                    # This case can be for things like temperature ranges without a file
                    # Example: {'type': 'float', 'low': 20, 'high': 100}
                    # bayes_opt handles this directly, no need to process here
                    # We just need to ensure it's in pbounds
                    if 'low' in details and 'high' in details:
                        self.pbounds[name.lower()] = (details['low'], details['high'])
                        opt_logger.info(f"  - Registered SEARCH component '{name}' with range [{details['low']}, {details['high']}].")
            elif mode == 'fixed':
                opt_logger.info(f"  - Registered FIXED component '{name}'.")
    ...
    def _load_data_for_component_if_needed(self, name: str):
        ...
        try:
            ...
            opt_logger.error(f"FATAL: Error loading file '{file_path}' for component '{name}': {e}")
            raise
    ...
    def build_reaction_df(self, dynamic_indices: Dict[str, int], fixed_components: Optional[Dict[str, str]] = None,
                ...
                if 'row_index' in details:
                    ...
                    if df is not None:
                        ...
                        if 0 <= row_idx < len(df):
                            ...
                            opt_logger.info(f"  - Fixed component '{name}' using row {row_idx} (Index={idx})")
                        else:
            ...
            if 'data' in component and component['data'] is not None and idx is not None:
                ...
                if display_col:
                    ...
                    if (feature_gen_config and
                        ...
                        opt_logger.info(f"  - Added SMILES column: {capitalized_name} = {smiles_value}")
                ...
                if details.get('is_feature_source'):
                    ...
                    if name.lower() == 'temperature':
                        for col, val in feature_data.to_dict().items():
                            if col == 'Temp':
                                data_row['Temp'] = val
                                opt_logger.info(f"  - Added temperature feature: Temp = {val}")
                    else:
                        ...
                        opt_logger.info(f"  - Added {len(prefixed_features)} features with prefix '{prefix}'")

        # 3. Generate features for SMILES columns - THIS IS THE KEY FIX
        if smiles_for_feature_gen and feature_gen_config:
            opt_logger.info(f"Generating features for SMILES columns: {list(smiles_for_feature_gen.keys())}")
            ...
            for smiles_col, smiles_value in smiles_for_feature_gen.items():
                if smiles_col in feature_gen_config.get('per_smiles_col_generators', {}):
                    ...
                    if not generated_df.empty:
                        ...
                        opt_logger.info(f"  - Generated {len(feature_data)} features for {smiles_col}")

        # 4. Add Index column to match training data structure
        # The training data includes an 'Index' column as a feature
        if 'Index' not in data_row:
            data_row['Index'] = 1  # Use a dummy index value for optimization

        # 5. Add dummy target column if not present
        ...
        opt_logger.info(f"Built reaction DataFrame with {len(data_row)} total columns")

        # --- START OF FIX ---
        # The following line was printing the entire list of columns, flooding the log.
        # It has been commented out to keep the log clean. The total column count above is sufficient.
        # opt_logger.info(f"DataFrame columns: {sorted(list(data_row.keys()))}")
        # --- END OF FIX ---
