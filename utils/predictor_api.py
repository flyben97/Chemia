...
def load_prediction_artifacts(
    ...
    """
    统一加载预测所需的 artifacts（配置、模型、scaler、label_encoder）。
    支持实验目录模式（run_dir + model_name）或文件路径模式。

    Returns:
        (config, model, scaler, label_encoder)
    """
    from utils.io_handler import (
        load_config_from_path, load_model_from_path,
        load_scaler_from_path, load_label_encoder_from_path,
    ...
    if run_dir:
        config = load_config_from_path(os.path.join(run_dir, "run_config.json"))
        ...
        scaler = load_scaler_from_path(os.path.join(run_dir, 'data_splits', "processed_dataset_scaler.joblib"))
        if task_type != 'regression':
            le_path = os.path.join(run_dir, 'data_splits', "processed_dataset_label_encoder.joblib")
            if os.path.exists(le_path):
                label_encoder = load_label_encoder_from_path(le_path)
    else:
    ...
    return config, model, scaler, label_encoder
