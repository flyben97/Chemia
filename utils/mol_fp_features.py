...
def calculate_molecular_features(smiles, fp_type=None, descriptors=False, radius=2, nBits=2048):
    ...
    Note:
        Fixed dimensions for different fingerprint types:
        - MACCS: always 166 bits
        - Morgan/RDKit/AtomPair/Torsion: configurable via nBits parameter
        - RDKit descriptors: ~200+ features (depends on RDKit version)
    """
    ...
    df = pd.DataFrame([row], index=[smiles])  # type: ignore
