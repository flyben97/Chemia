...
def load_model_components(model_source_config):
    ...
    if 'source_run_dir' in model_source_config and 'model_to_use' in model_source_config:
        ...
        scaler_path = paths.get('scaler_file', None) # Optional
    ...
    scaler = load_scaler_from_path(scaler_path) if scaler_path and os.path.exists(scaler_path) else None
...
def main(config_path: str):
    console.rule("[bold]Chemia Optimization Only Workflow[/bold]")

    # 动态查找配置文件
    actual_config_path = find_config_file(config_path, project_root)
    console.print(f"Loading optimization configuration from: [cyan]{actual_config_path}[/cyan]")
    ...
    opt_logger.info("Chemia Reaction Optimizer Started (Optimization Only)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Chemia Bayesian optimization on a pre-trained model.")
    parser.add_argument(
        '--config',
        type=str,
