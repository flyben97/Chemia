#!/usr/bin/env python3
"""
CHEMIA Standalone Prediction Runner

This script provides a user-friendly interface for making predictions with trained CHEMIA models.
...
class PredictionRunner:
   ...
   ██      ██   ██ ██   ██ ██         ██
    ██████ ██   ██ ██   ██ ██         ██
        ...
        """
        console.print(Panel(banner_text, style="bold blue", title="CHEMIA Framework"))

    def load_artifacts_from_experiment(self, run_dir: str, model_name: str) -> bool:
        """Load model artifacts from an experiment directory using the unified API."""
        try:
            ...
            self.training_config, self.model, self.scaler, self.label_encoder = load_prediction_artifacts(
    ...
    def load_artifacts_from_files(self, model_path: str, config_path: str,
        ...
        """Load model artifacts from direct file paths using the unified API."""
        try:
            ...
            if encoder_path and self.label_encoder:
                console.print("   • Label encoder loaded: [green]✓[/green]")
            return True
        except Exception as e:
            console.print(f"[bold red]❌ Error loading artifacts:[/bold red] {str(e)}")
...
def interactive_mode():
    """Interactive mode for guided prediction setup"""
    console.print(Panel("Welcome to CHEMIA Interactive Prediction Mode", style="bold blue"))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CHEMIA Standalone Prediction Runner",
