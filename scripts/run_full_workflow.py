...
from utils.config_finder import find_config_file
...
def main(config_path: str):
    ...
    # 动态查找配置文件
    actual_config_path = find_config_file(config_path, project_root)
    console.print(f"Loading workflow configuration from: [cyan]{actual_config_path}[/cyan]")
    ...
    try:
        ...
        # --- ROBUSTNESS FIX: Handle saving and logging ---
        if top_results is not None and not top_results.empty:
            ...
            console.print(f"\n[green]✓ Report with top {len(top_results)} conditions saved to:[/green] [cyan]{output_csv_path}[/cyan]")
        else:
            opt_logger.warning("Optimization finished but produced no valid results to save to CSV.")
            console.print("[yellow]Note: No CSV report was generated as optimization did not yield valid conditions.[/yellow]")

    except Exception as e:
        opt_logger.error("A critical error occurred during the optimization phase.", exc_info=True)
        console.print(f"[bold red]An error stopped the optimization: {e}[/bold red]")

    console.rule("[bold green]✓ End-to-End Workflow Finished[/bold green]")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the full Chemia workflow: Train -> Select -> Optimize.")
    parser.add_argument(
        'config_path',
        nargs='?',  # Makes it optional
        type=str,
        default=None,
        help="Path to the full workflow configuration YAML file (positional argument)."
    )
    parser.add_argument(
        '--config',
        type=str,
        default="config_full_workflow.yaml",
        help="Path to the full workflow configuration YAML file (flag argument)."
    )
    args = parser.parse_args()

    # Use positional argument if provided, otherwise use --config flag
    config_file = args.config_path if args.config_path else args.config
    main(config_file)
